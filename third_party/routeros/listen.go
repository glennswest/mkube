...
func (c *Client) ListenArgsQueueContext(ctx context.Context, sentence []string, queueSize int) (*ListenReply, error) {
	...
	// mkube fork fix: register the tag BEFORE the sentence is sent, so a
	// fast first reply cannot beat the registration and be dropped by
	// asyncLoop (same race as RunArgsContext, see run.go).
	c.mu.Lock()
	defer c.mu.Unlock()
