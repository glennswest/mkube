...
func (c *Client) RunArgsContext(ctx context.Context, sentences []string) (*Reply, error) {
	...
	// mkube fork fix: register the tag BEFORE the sentence is sent. Upstream
	// registers after EndSentence, so a fast reply could reach asyncLoop
	// before the tag existed — the reply was dropped ("cannot find tag") and
	// the caller blocked forever on a.reC. Observed live against a LAN
	// RouterOS (sub-ms RTT) under concurrent load.
	c.mu.Lock()
	if c.tags == nil {
		c.mu.Unlock()
	...
	for {
		select {
		case <-ctx.Done():
			// mkube fork fix: upstream called c.r.Cancel() here, which makes
			// the SHARED reader return io.EOF — tearing down the async loop
			// and failing every other in-flight request on the connection
			// because one request's context expired. Instead, abandon only
			// this request: deregister its tag (a late reply is then safely
			// ignored by asyncLoop) and leave the connection alone.
			c.mu.Lock()
			if c.tags != nil {
				delete(c.tags, a.tag)
			}
