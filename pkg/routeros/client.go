...
type FileDisk struct {
	...
	ISCSIUser      string `json:"iscsi-user,omitempty"`
}

// CreateISCSITarget creates a file-backed disk and enables iSCSI export with optional CHAP auth.
// Returns the .id of the created disk.
func (c *Client) CreateISCSITarget(ctx context.Context, name, filePath, chapUser, chapPassword string) (string, error) {
	...
	// Step 2: Enable iSCSI export with optional CHAP authentication
	settings := map[string]string{
		".id":          diskID,
		"iscsi-export": "yes",
	}
	if chapUser != "" {
		settings["iscsi-user"] = chapUser
	}
	if chapPassword != "" {
		settings["iscsi-password"] = chapPassword
	}
