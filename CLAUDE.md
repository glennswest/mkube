...
3. **Registry push notifications to mkube-update**: Webhook/watch instead of polling. **Superseded by `enhancements/stormblock-registry.md`** (sbregistry absorbs mkube-update; fires webhooks by construction) — only do standalone if that spec stalls.
