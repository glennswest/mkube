...
- [ ] (2026-08-10) **CoW catalog implementation**: Phase 0/0b ANSWERED — verdict **supported** (probe run 11, `POST /api/v1/probes/cow`). Proven recipe: (1) stormblock clone volume attach+mount; (2) SEEDER container (`file=<docker-save>` `root-dir=<clone mount>`) — RouterOS's own untar populates the volume with exec bits intact; (3) per-pod container = tiny generic docker-save stub `file=` + clone mounted via `mounts=` + entrypoint into the mount — the binary RAN from the mounted clone. Hard facts: imageless add rejected by design; ROS renames root-dir to `.backup` and extracts fresh (in-place preservation dead); mount `dst=/` rejected (full distros need the symlink-farm stub or per-dir mounts); container REMOVAL wipes its root-dir (seal golden templates BEFORE removing seeders; or keep seeders as never-started pins). Build-out: golden volume per image digest + fstemplate seal + `from_template` clone per pod in CreatePod; recreate cost becomes clone+attach+mount+2KB-stub ≈ seconds for any image size.
