...
- mkube-update tarball format fix (crane.Save produced OCI format with compressed layers; RouterOS needs docker-save v1 with uncompressed layers, repositories file, VERSION/json per layer)
...
9. **iSCSI CDROM CRD**: Implemented and deployed. Needs end-to-end testing with RouterOS iSCSI targets once ISO upload and device discovery are verified.
10. **microdns resilience**: Validate that microdns (DNS) containers survive mkube failures without going down. DNS is critical infrastructure and should not depend on mkube being healthy. Currently if mkube-update pushes a bad image update, DNS containers can be destroyed along with mkube.
