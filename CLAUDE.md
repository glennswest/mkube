...
### In Progress
- [ ] (2026-08-09) **DNS client: replace batch blacklist with health-poll alive gate** (`pkg/dns/client.go`): The BeginBatch/EndBatch record cache + `failedEndpoints` blacklist is client-global shared state raced by three concurrent batchers (`provider.go` alias registration, `network/manager.go` infra sync, `consistency.go` cleanup) — produced ~9,100 false "previously failed this batch" skips for healthy g8/gw endpoints on 2026-08-10 and left those zones unregistered most cycles. Fix: remove batch mode entirely (LAN calls are ~2ms; cache is a non-optimization) and gate endpoint ops on a TTL-cached `GET /api/v1/health` alive probe (microdns is database-driven — alive = API up). Log state transitions only. microdns-side spec (health reflects DB readability) goes to `microdns/enhancements/`.
