...
- [ ] (2026-08-10) **CoW container root-dir probe (Phase 0)**: RouterOS capability test — create a container whose `root-dir` points at a pre-populated stormblock-mounted rootfs with no `file=`/`remote-image=`. Decides between the direct golden+clone catalog (untar once per digest, `from_template` clone per pod) and the stormblock-registry fallback. All plumbing (attach/format/mount/idempotency) proven by the sb-test work.
