...
11. ~~**RouterOS native API reconnect race**~~ **FIXED 2026-08-09** (`pkg/routeros/client.go`): Root cause was error misclassification, not the reconnect itself — a per-request context deadline was treated as connection death and closed the shared multiplexed connection, cascading failures across every in-flight request (observed 2026-05-15 and again live 2026-08-09 22:57Z, where it blocked recreation of ~10 pods for 25+ min). `nativeRun` now keeps the shared connection up on ctx deadline/cancel (reconnecting only after 3 consecutive deadline expiries), retries real connection errors 4× with backoff, and wraps the final error with the cause.
