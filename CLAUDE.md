...
until ALL of the following land (code complete 2026-08-27, UNTESTED — dev.g8.lo unreachable; build+test+deploy pending):
1. [x] **Ownership model (#18/#26)**: mkube may only remove veths/bridge-ports it
...
2. [x] **Bridge-port leak + GC (#14)**: RemoveVeth/recreateVethForBridge remove the
   static bridge-port entry with the veth; GC pass drops dangling `*XX` entries.
3. [x] **Churn bound (#26)**: per-interface failure backoff + cap on the
   add→start→fail→remove loop; after cap, pod goes Failed, loop stops.
4. [x] **Pull-verify-then-cut-over (#26)**: never destroy a container before the
...
- [ ] (2026-08-10, **updated 2026-08-26**) **CoW catalog — seeding and
  ...
  the other CoW-ready images. **2026-08-26 rose1 freeze (post-mortem):**
  the node locked up hard at 22:34:43Z — mkube's persisted log stops
  mid-stream across all goroutines with no panic, stormd's liveness never
  fired, every disk-backed service died (mkube API, all microdns REST/sqlite)
  while ping and cached UDP :53 kept answering — i.e. a RouterOS
  storage-path wedge, not an mkube crash; RouterOS's RAM-only logs were lost
  to the power cycle. In flight at the time: `gt/cow-test` in a permanent
  create→5m-golden-wait→fail→self-heal→retry loop (sbregistry never published
  the nats golden — it was being killed mid-build by `image-policy: auto`
  redeploys, discarding progress each restart, with 9.7 MB layer unpacks
  taking 2–5 min through stormblockmk). Leading hypothesis: same-host
  loopback storage deadlock (rose1 mounts NVMe-TCP whose target,
  stormblockmk, is a container on rose1 — kernel writeback to the mount
  needs a local userspace process to make progress). Actions taken:
  cow-test deleted; 30s resource trace added (v6.5.0) so the next freeze
  leaves a memory/CPU/disk trail; golden wait loop made loud (v6.5.0);
  debuggability spec filed in stormblock-registry
  (`enhancements/debuggability-and-graceful-restart.md`). **The reboot-soak
  data point is in**: after the power cycle, netwatch (CoW) recovered to
  Running by reconcile alone. Structural risk to keep in mind: the
  NVMe-TCP-target-on-the-same-node topology. Original
...
- [ ] (2026-08-06) **g16 follow-ups**: `fedora-siov` (Mellanox b8:59:9f:52:23:46) identified as the **pvex Mellanox card** — an SR-IOV test VM on the Proxmox node; owner will convert it to a plain Linux NIC (MAC will change), then give it a static below 16.100; discover server7's b-port MAC and fill the reserved 192.168.16.113; TODO #17 (network-annotation strand bug) and #18 (failover DNS per server, prefer lower IP).
