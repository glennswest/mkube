...
- [ ] (2026-08-10) **CoW catalog — Phase 0b (stub-tarball variant)**: Phase 0 ANSWERED 2026-08-10 via `POST /api/v1/probes/cow`: RouterOS **rejects** imageless container/add with `failure: No file or remote-image` — the direct "point root-dir at a clone" path is off the table. Next probe: `container/add file=<tiny stub tarball> root-dir=<clone mount>/rootfs` — does RouterOS extract the stub INTO the pre-populated (cloned) rootfs leaving existing content intact, then start it? If yes, CoW is achieved as clone (instant) + one-file stub untar (instant): effectively untar-once. The probe harness (`pkg/provider/cow_probe.go`) has all the pieces including device-side `/tool/fetch` payload delivery. If no, the stormblock-registry design carries the fallback.
