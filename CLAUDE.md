...
### In Progress
- [ ] (2026-08-10) **stormblock NVMe-TCP switchover**: stormblockmk v0.3.0 accepts `protocol` on volume create (spec `stormblockmk/enhancements/volume-create-protocol.md` — implemented). Once the v0.3.0 image is pushed to the registry (pod has image-policy auto), set `storage.stormblock.transport: "nvme-tcp"` in rose1-config, `make deploy-config`, restart mkube, rerun the gt/sb-test cycle. mkube's attach path (`/disk add type=nvme-tcp`) already handles it.
