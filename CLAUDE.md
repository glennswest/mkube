...
### In Progress
- [ ] (2026-08-10) **stormblock NVMe-TCP switchover** — BLOCKED ONLY ON THE stormblockmk v0.3.0 IMAGE PUSH. Verified live by `POST /api/v1/probes/nvme` (2026-08-10): **RouterOS 7.22.2 accepts `/disk add type=nvme-tcp`** (it created a disk row against a deliberately absent target), so the initiator-support risk the spec flagged is retired; and the deployed **stormblockmk is 0.2.0**, which ignores `protocol` and exports iSCSI. mkube's side is already live: rose1-config carries `transport: "nvme-tcp"`, mkube sends `protocol` on volume create and attaches whatever the export reports (host/port split fix included). When the v0.3.0 image is pushed, the pod auto-rolls on digest and the next volume created comes up NVMe. Note: existing volumes keep iSCSI — transport is fixed at export creation, so moving one means recreating its PVC.
- [ ] (2026-08-10) **CoW catalog — blocked on volume durability, NOT on RouterOS capability.** Phase 0/0b proved the model works: a container runs its binary from a mounted stormblock clone (generic 5 KB docker-save stub as `file=` + clone mounted at `/payload` + entrypoint rewritten). Implemented end to end (`pkg/provider/cow_catalog.go`, opt-in `vkube.io/image-mode: cow`): golden template create→format→seed→seal, clone per pod with its own UUID/label/clean-state, attach, mount, container start. **What fails:** a volume RouterOS seeds ends up without a usable filesystem — its raw volume will not mount afterwards (`fs=-`) and clones of the sealed template mount as valid-but-EMPTY ext4 despite the full 60 MB being allocated. Ruled out by measurement: 45 s writeback settle, clean-flag restore, `/disk/eject` (hardware-only), `/disk/unmount` (absent), `/disk/disable` (accepted, no effect), and an explicit SCSI SYNCHRONIZE CACHE from our own initiator (succeeds, no effect), plus per-clone identity. Leading theory is partial 4 MB slab-slot durability (bulk data fills whole slots and survives; scattered 4 KB ext4 metadata does not) — filed with a RouterOS-free reproduction in `stormblock/enhancements/routeros-seeded-volume-loses-filesystem.md`. rose1 runs `goldenSource: sbregistry`; mkube waits for a sealed `img-<digest12>` template and clones it. **Caveat for the registry work: if partial-slot durability is the cause, sbregistry inherits it — run the reproduction first.** Probes retained: `/api/v1/probes/{cow,nvme,layers,layerdir,format,lsmount,inspect}`.
