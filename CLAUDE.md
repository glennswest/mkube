...
16. **Pod network/static-ip annotation change strands the pod** (`pkg/provider/provider.go`): Observed live 2026-08-06 21:16Z moving infra/bmh-operator and g10/fastregistry from g10 to g16: PATCHing `vkube.io/network` + `vkube.io/static-ip` triggered a blue-green "Updated" event, then the container was torn down (veth released, files removed) and **never recreated** — pod left untracked, reconcile loop skipped it (`IsPendingOrProcessing` / worker wedged), `images/redeploy` returned `queued: null`, and only a full mkube restart recovered. Needs: an explicit network-change path in UpdatePod (teardown + CreatePod on the new network, serialized in the pod worker), and the reconcile loop must never permanently skip a desired pod whose containers are gone.
...
### In Progress
- [ ] (started 2026-08-06) **g16 PXE + host renumbering** — move the bare-metal fleet from g10 addressing onto the g16 flat /20 fabric:
  1. Code: add `hostname` override to `BMHNICSpec` so b-port reservations can be named `serverNb` (not forced `{hostname}-{role}`). Push → CI builds → mkube self-updates.
  2. Config: add `g16` networks entry to `deploy/rose1-config.yaml` (mirrors live network object) with PXE dhcp options (nextServer 192.168.10.200 = bmh-operator, undionly.kpxe / ipxe.efi); move g16 dynamic DHCP range off the host block to 192.168.20.1–192.168.30.254.
  3. Live: PATCH mkube g16 network + microdns g16 pool (range + PXE opts).
  4. IP plan (user-approved): blades server1–8 a/b adjacent pairs — serverNa = 16.100+2(N−1) (1G boot NIC, PXE), serverNb = odd (Mellanox 40G, no gateway/PXE): 16.100/101 … 16.114/115. Storage server9 (R730xd) = 192.168.16.120. Hostname scheme: explicit `serverNa`/`serverNb`; server9 stays `server9`.
  5. BMH updates drive it: server9 first (user needs it — PXE boot on g16 + card inventory via iDRAC Redfish 192.168.11.18 + baremetalservices agent), then server1–8 (spec.hostname=serverNa, network=g16, new IP, NICs\[b\] once new mkube is live). server7b MAC unknown — discover during rollout.
  6. Cleanup after verification: stale g10 b-side reservations (a-side auto-removed by BMH network move). g10 network entry itself stays (migration reference per collapse note).
